...
const QuestionForm = () => {
    ...
    try {
      if (editingId) {
        ...
        if (res.data.success) {
          setQuestions((prev) =>
            prev.map((q) =>
              q._id === editingId ? { ...formData, _id: editingId } : q
            )
          );
  ...
  return (
    ...
    >
      ...
      >
        <h2
          style={{ color: "white", paddingBottom: "20px", textAlign: "center" }}
        >
          Kindly add your question below
        </h2>
        <input
        ...
        <tbody>
          {questions.map((q, index) => (
            <tr key={q._id}>
              ...
              <td style={styles.td}>
                <button
                  style={{ ...styles.button, background: "#007cf0" }}
...
const styles = {
  ...
  button1: {
    width: "48%",
    padding: "10px",
    background: "linear-gradient(to right, #754086ff, #7928ca)",
    color: "#fff",
    border: "none",
    borderRadius: "8px",
    fontWeight: "bold",
    fontSize: "14px",
    cursor: "pointer",
    height: "42px", // matches inputHalf height
